...
<template>
    <li class="md:grid md:grid-cols-5 md:gap-4">
        <p class="text-slate-400 text-sm opacity-75 mt-1 mb-2 md:mb-0">{{ dateRange }}</p>
        <div class="md:col-span-4">
            <a :href="companyUrl" 
               target="_blank" 
               rel="noopener noreferrer" 
               class="text-slate-200 font-bold hover:text-teal-300 group">
                {{ title }} — <span class="inline-block">{{ company }}<svg 
            ...
            <div v-if="skills.length" class="mt-3 flex flex-wrap gap-2">
                <span v-for="skill in skills" 
                      :key="skill" 
                      class="px-3 py-1 text-sm rounded-full text-teal-300 bg-teal-300/10">
                    {{ skill }}
                </span>
            </div>
        </div>
